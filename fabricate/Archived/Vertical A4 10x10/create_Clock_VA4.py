...
PCB_OFF = .05 * inch
HEIGHT = 300 * mm + 2 * PCB_OFF
WIDTH = 210 * mm + 2 * PCB_OFF
BEND_R = 10 * mm
EDGE_THICKNESS = 3 * mm
...
__version__ = '30_1.0' # refers to mechichanical version.
...
def create_baffles():
    ...
    can = canvas.Canvas('%s/baffles_VA4_%s.pdf' % (directory, __version__),
                        pagesize=(W, H))
    can.translate(PAGE_MARGIN, PAGE_MARGIN)
    can.setFont('Courier', 15)

    h_baffle.translate(0, .5*inch)
    can.drawCentredString(2 * inch, .25 * inch, 'H baffle: 11 per clock')

    v_baffle.translate(0, 2*inch)
    can.drawCentredString(2 * inch, 1.75 * inch, 'V baffle: 11 per clock')
...
#    localizer.drawOn(can, linewidth)
    can.drawCentredString(2.5 * inch, -.75* inch, 'ClockFOUR_VA4 Baffles 2mm BLACK Acrylic')
