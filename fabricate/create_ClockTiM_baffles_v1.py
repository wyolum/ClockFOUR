...
PCB_OFF = .05 * inch
HEIGHT = 210 * mm + 2 * PCB_OFF
WIDTH = 297 * mm + 2 * PCB_OFF
BEND_R = 10 * mm
EDGE_THICKNESS = 3 * mm
...
__version__ = 'TiM_1.0' # refers to mechichanical version.
...
fontdir = 'C:\Users\David\Documents\GitHub\ClockFOUR\fabricate\fonts\Cantarell-Bold.ttf'
fontdir = './fonts/'
...
def create_faceplate(basename, style, case, font, fontsize, reverse=True, color=None,
    ...
    if reverse:
        ...
        can.drawCentredString(W / 2 + MARGIN, H - 7.5 * MARGIN, "BACKWARDS, DO NOT PRINT")
...
h_baffle = TiM_h_baffle(BAFFLE_H,
                         BAFFLE_T,
                         n_notch=17,
                         delta=DX,
...
v_baffle = TiM_v_baffle(BAFFLE_H,
                         BAFFLE_T,
                         n_notch=9,
                         delta=DY,
                         overhangs=(5*mm, 5*mm),
...
def create_baffles():
    ...
    can.drawCentredString(2 * inch, .25 * inch, 'H baffle: 9 per clock')

    v_baffle.translate(0, 2*inch)
    can.drawCentredString(2 * inch, 1.75 * inch, 'V baffle: 17 per clock')

    localizer.translate(0, 3 * inch)
    can.drawCentredString(1.75 * inch, 3.2 * inch, 'localizer: 0 per clock')

    h_baffle.drawOn(can, linewidth)
    v_baffle.drawOn(can, linewidth)
    localizer.drawOn(can, linewidth)
    can.drawCentredString(2.5 * inch, -.75* inch, 'ClockTiM_v1 Baffles 0.06" BLACK Acrylic')
