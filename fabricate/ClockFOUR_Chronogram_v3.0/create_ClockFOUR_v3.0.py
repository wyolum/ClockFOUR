...
__version__ = 'C4_v3.0' # refers to mechichanical version.
...
def create_baffles():
    can = canvas.Canvas('%s/baffles_%s.pdf' % (directory, __version__),
                        pagesize=(WIDTH + 2 * MARGIN, HEIGHT + 2 * MARGIN))
    can.drawCentredString((MARGIN + WIDTH + MARGIN)/ 2, HEIGHT + 1.5 * MARGIN, "ClockFOUR baffles %s" % __version__)
    can.translate(MARGIN, MARGIN)
...
## v_baffle
    ...
    can.drawCentredString(50 * mm, -10 * mm, 'ClockFOUR_v3 Baffles 2mm BLACK Acrylic')
